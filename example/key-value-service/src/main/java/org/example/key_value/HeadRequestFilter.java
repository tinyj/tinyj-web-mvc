...
import org.tinyj.web.mvc.HttpRequestFilter;
...
class HeadRequestFilter implements HttpRequestFilter {
  @Override
  public void filter(HttpServletRequest req, HttpServletResponse res, HttpRequestHandler next) throws Exception {
    if (req.getMethod().equals("HEAD")) {
      next.handle(new HeadRequest(req), new HeadResponse(res));
    } else {
      next.handle(req, res);
    }
  }

  public static class HeadRequest extends HttpServletRequestWrapper {
    public HeadRequest(HttpServletRequest req) {
      super(req);
    }
